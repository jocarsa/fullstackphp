...
window.onload = function(){                                     // Solo ejecuto Javascript cuando el documento haya cargado
    ...
    .then(function(datos){                                      // Cuando el paso anterior haya sido completado
        datos.forEach(function(restriccion){
    ...
    for(let i = 0;i<10;i++){
        let datos = {}
        for(let j = 0;j<Math.round(Math.random()*10)+5;j++){
            datos[j] = Math.random()*100
        }

        grafica("graficas",datos,[0,139,139],'Título de la gráfica');
    }
