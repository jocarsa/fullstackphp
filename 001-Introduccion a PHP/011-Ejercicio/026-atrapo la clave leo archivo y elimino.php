...
<?php
    ...
    if(isset($_GET['operacion']) && $_GET['operacion'] == "eliminar"){
        ...
        foreach($coleccion as $elemento){                       // PAra cada uno de los elementos
            if($elemento['clave'] == $_GET['clave']){           // En el caso de que la clave coincida
                array_splice($coleccion,$contador,1);                 // Elimina el elemento de la coleccion
                break;                                          // Sal del bucle
            }else{                                              // En caso contrario
                $contador++;                                    // Sube una unidad al contador
            } 
        }
                ...
                <tbody>
                    ...
                    foreach($coleccion as $objeto){                         // Para cada elemento (originalmente eran objetos)
                        ...
                        foreach($objeto as $clave=>$valor){                 // Y ahora para cada registro
                            if($clave == "clave"){$miclave = $valor;}
                            echo '<td>'.$valor.'</td>';                     // Le añado una celda
                        }
                        echo '<td><a href="?operacion=eliminar&clave='.$miclave.'">🗑️</a></td>';
